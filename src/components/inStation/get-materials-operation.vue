...
const filterOption = (input: string, option: any) => {
  return (
    option.materialCode.toLowerCase().indexOf(input.toLowerCase()) >= 0 ||
 ...
 */
function init() {
  getBatchListByWorksheetCode(prop.sheetMessage?.workSheetCode)
    .then(({ data: { code, data, msg } }: any) => {
      if (code == 200) {
...
const logs = ref<any[]>([]);
function queryLogs() {
  getAllOutWarehouseList(prop.sheetMessage?.workSheetCode).then(
    ({ data: { code, data, msg } }: any) => {
      if (code == 200) {
        logs.value = data;
      }
    }
...
<template>
  <a-button type="primary" @click="addLine()"> 添加 </a-button>
  <a-form
    ref="formRef"
  ...
  >
    <a-space direction="vertical" style="width: 100%; margin-top: 1em">
        ...
        <a-card>
          ...
          >
            <a-select
              ...
              :options="materialArr"
              @change="
                (_value: string, _item_selecte: any) => selectedChange(item)(_value, _item_selecte)
              "
            />
          </a-form-item>
      ...
      <a-button
        type="primary"
        html-type="submit"
        :disabled="listOfMaterials.length === 0"
  ...
  <a-timeline style="margin-top: 3em">
    <a-timeline-item v-for="item of logs" :key="item.id" color="green">
      <a-descriptions bordered>
        <a-descriptions-item label="砖坯类型">{{ item.applyType }}</a-descriptions-item>
        <a-descriptions-item label="领用数量">{{ item.stockNumber }}</a-descriptions-item>
        <a-descriptions-item label="破碎数量">{{ item.brokenNumber }}</a-descriptions-item>
